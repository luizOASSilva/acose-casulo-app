...
class ActivityController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('q')) {
            ...
            $query->whereHas(
                ...
                function ($publicationQuery) use ($search) {
                    $publicationQuery->where(function ($query) use ($search) {
                        $query
        ...
        if ($request->filled('weekday')) {
            $query->whereHas(
                'schedules',
                function ($scheduleQuery) use ($request) {
                    $scheduleQuery->where(
                        'weekday',
    ...
    public function schedules()
    {
        return ActivitySchedule::query()
            ...
            ->orderBy('start_time')
            ->get()
            ->map(fn (ActivitySchedule $schedule) => [
                'id' => $schedule->id,
                'activity_id' => $schedule->activity_id,
                'activity_title' => $schedule
                    ->activity
                    ?->publication
                    ?->title,
    ...
    public function store(StoreActivityRequest $request)
    {
        ...
        $activity = DB::transaction(
            function () use ($validated, $request) {
                ...
                $activity
                    ->schedules()
                    ->createMany($validated['schedules']);
    ...
    public function update(
        ...
        $activityModel = DB::transaction(
            function () use ($validated, $activityModel) {
                $activityModel->load([
                    'publication.media',
                    'schedules',
                ]);
                ...
                if (
                    ...
                    $media = $activityModel->publication?->media;
    ...
    public function destroy(string $activity)
    {
        ...
        DB::transaction(
            function () use ($activityModel, $snapshot) {
                ActivityAuditLogger::deleted(
                    $activityModel,
                    $snapshot,
                    request()
                );
    ...
    private function findActivityByIdOrSlug(
        string $activity
    ): Activity {
    ...
    private function weekdayLabel(?string $weekday): string
    {
        $labels = [
